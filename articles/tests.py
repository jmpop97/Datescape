...
class ArticleDetailAPIViewTest(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.articles = []
        cls.user = User.objects.create_user("test2@test.com", "test2", "test")
        cls.locaton = KakaoMapDataBase.objects.create(jibun_address="경기 평택시 팽성읍 본정리 15-3",
        ...
        for i in range(10):
            cls.articles.append(
                Article.objects.create(
                    user=cls.user, title=cls.faker.sentence(), content=cls.faker.text(), location=cls.locaton
                )
            )
...
class CommentAPIViewTest(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.article = Article.objects.create(
                    user=cls.writer, title="testcase", content="testcode",score=4, location=cls.locaton
                )
        cls.comment_data = {
            "comment": "comment test",
        }
        cls.comment = Comment.objects.create(article=cls.article, writer = cls.user, comment="comment test")
        cls.emoticon_data = {
            "title": "test emoticon",
            "images": [],
            "db_status_choice": 0,
...
class LocationListAPIViewTest(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.articles = []
        cls.user = User.objects.create_user("test2@test.com", "test2", "test")
        cls.locaton = KakaoMapDataBase.objects.create(jibun_address="서울 구로구 구로동 435",
...
class SearchAPIViewTest(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.articles_data = [{ "title": "testcase",
            "content": "testcode",
            "score":4,
            "tags":[cls.tag1.id, cls.tag2.id]},{ "title": "case",
            "content": "code",
            "score":5,
            "tags":[cls.tag3.id, cls.tag2.id]},{ "title": "testcase",
            "content": "testcode",
            "score":5,
        ...
        for i in cls.articles_data:
            cls.article = Article.objects.create(user=cls.user, title=i["title"], content=i["content"],score=i["score"],location=cls.locaton)
